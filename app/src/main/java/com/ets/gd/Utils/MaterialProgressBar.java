...
public class MaterialProgressBar extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.MaterialProgressBar, defStyleAttr, 0);

        final float density = getContext().getResources().getDisplayMetrics().density;

        mBackGroundColor = a.getColor(
                R.styleable.MaterialProgressBar_background_color,
                DEFAULT_CIRCLE_BG_LIGHT);

        int mProgressColor = a.getColor(
                R.styleable.MaterialProgressBar_progress_color,
                DEFAULT_CIRCLE_BG_LIGHT);
        ...
        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.MaterialProgressBar_progress_stoke_width,
        ...
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.MaterialProgressBar_progress_text_size,
                (int) (DEFAULT_TEXT_SIZE * density));
        int mTextColor = a.getColor(
                R.styleable.MaterialProgressBar_progress_text_color,
                Color.BLACK);

        mShowArrow = a.getBoolean(R.styleable.MaterialProgressBar_show_arrow,
                false);
        mCircleBackgroundEnabled = a.getBoolean(
                R.styleable.MaterialProgressBar_enable_circle_background, true);
        ...
        int textVisible = a.getInt(
                R.styleable.MaterialProgressBar_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
        ...
        if (getBackground() == null && mCircleBackgroundEnabled) {
            ...
            if (elevationSupported()) {
                ...
                ViewCompat.setLayerType(this, ViewCompat.LAYER_TYPE_SOFTWARE,
                        mBgCircle.getPaint());
                mBgCircle.getPaint().setShadowLayer(mShadowRadius,
